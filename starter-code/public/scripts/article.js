...
Article.numWordsAll = () => {
  return Article.all.map(ele => ele.body.split(' ')).reduce((acc, item) => acc + item.length, 0);
};

// DONE: Chain together a `map` and a `reduce` call to produce an array of unique author names. You will
// probably need to use the optional accumulator argument in your reduce call.
Article.allAuthors = () => {
  return Article.all.map(ele => ele.author).reduce((acc, cur) => acc.indexOf[cur] === -1)
};
...
Article.numWordsByAuthor = () => {
  return Article.allAuthors().map(author => ({
    name: author,
    words: Article.all.filter(cur => cur.author === author).map(ele => ele.body.split(' ')).reduce((a, b) => a + b.length)
  }));
